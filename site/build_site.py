#!/usr/bin/env python3
"""Generates docs/index.html (GitHub Pages) and the design-canvas artboards from one source."""
import os, pathlib
...
CSS = """
:root{--bg:#f7f5f1;--bg2:#efece6;--fg:#1c1b19;--fg2:#5f5c56;--line:#e0dcd4;--card:#ffffff;--accent:#c96442;--accent-fg:#ffffff;--deck:#1c1c1e}
[data-theme=dark]{--bg:#161615;--bg2:#1f1f1e;--fg:#f2f0ec;--fg2:#a09c94;--line:#2c2b29;--card:#1d1d1c;--accent:#d97757;--accent-fg:#161615}
...
.mark{width:30px;height:30px;border-radius:8px;background:var(--accent);color:var(--accent-fg);display:grid;place-items:center;font-weight:700;font-size:17px}
...
/* No width cap: the copy runs the full page width, same as everything else.
   The lead paragraph keeps a reading measure — a 1040px line of 17px text is
   about 115 characters, which is past where the eye loses its place. */
.herocopy p{max-width:none}
.herocopy .lead2{max-width:none}
 /* One line. There is no CSS text-fit, so the size is chosen to fit the
    headline's 36 characters inside the 984px content width with margin to
    spare, and nowrap is lifted on narrow screens where it would overflow. */
.hero h1{font-size:clamp(30px,4.2vw,49px);line-height:1.05;font-weight:700;max-width:none;white-space:nowrap}
.hero p{font-size:20px;color:var(--fg2);margin:22px 0 30px;max-width:none}
.cta{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.btn{display:inline-flex;align-items:center;gap:10px;background:var(--accent);color:var(--accent-fg);font-weight:600;padding:15px 24px;border-radius:11px;font-size:17px}
.btn:hover{color:var(--accent-fg);filter:brightness(1.08)}
.btn.ghost{background:transparent;color:var(--fg);border:1px solid var(--line)}
.meta{font-size:14px;color:var(--fg2)}
.hero img{width:100%;height:auto;display:block}
/* The hero deck. Full page width so a 5x3 deck renders its keys around 130px
   and every label is readable — that legibility is the whole point of the image.
   No panel behind it: the render already carries its own deck body and margin,
   and wrapping that in a second frame just shrinks the keys again. */
.herodeck{margin:0 0 52px}
.herodeck img{display:block;width:100%;height:auto}
.deckshots{display:grid;gap:36px;margin-top:8px}
.deckshots figure{margin:0}
.deckshots img{display:block;width:100%;height:auto}
/* The Mini is six keys — at full page width they would be absurdly large. */
.deckshots figure.small img{max-width:430px}
.deckshots figcaption{margin-top:12px;font-size:13px;color:var(--fg2)}
...
.sheet img{display:block;width:100%;max-width:920px;margin:0 auto;height:auto}
...
@media (max-width:820px){.decks{grid-template-columns:1fr}.hero{padding-top:24px}.hero h1{font-size:38px}.grid3,.flow,.deckshots{grid-template-columns:1fr}.navlinks span{display:none}}
...
def dl_icon():
    return '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12m0 0 4-4m-4 4-4-4M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-2"/></svg>'

def body(img, toggle_button=True):
    """img(name) -> src attribute value."""
    tog = '<button class="toggle" id="theme" aria-label="Toggle dark mode" onclick="toggleTheme()"></button>' if toggle_button else '<button class="toggle" aria-label="Toggle dark mode"></button>'
    return f"""
<div class="wrap">
  <nav>
  ...
  <div class="hero">
    <div class="herocopy">
      ...
      <p class="meta" style="margin-top:16px">macOS · Stream Deck 6.9+ · one-click install: actions, a ready profile and the helper app. Free and open source (MIT). Also on the Elgato Marketplace; this GitHub build adds a <a href="https://ko-fi.com/K3K6RR4LY">Buy me a Ko-fi</a> key.</p>
    </div>
  </div>

  <!-- The keys are the product. Shown at the full width of the page, one deck
       rather than four scaled-down ones, because a hero that renders every key
       at 40px tells a visitor nothing about what they would be pressing. -->
  <div class="herodeck">
    <img src="{img('deck-hero.png')}" width="778" height="554" alt="A Stream Deck MK.2 running the bundled Claude profile: Allow once, Allow session, Always allow, Deny and Stop across the top row; Continue, Yes, Fix it, Commit and a green status light on the second; Claude, New chat, New session, Search and a usage tile on the third">
  </div>
  ...
  <section id="usage">
    ...
    <div class="sheet"><img src="{img('deck-usage-crop.png')}" width="774" height="550" alt="A Stream Deck MK.2 showing three usage bars across its keys: session 34%, weekly 68%, and a red Fable bar at 86% that is past its on-pace mark"></div>
    <table>
  ...
  <section id="decks">
    ...
    <div class="deckshots">
      <figure><img src="{img('deck-xl-crop.png')}" width="1218" height="712" alt="Stream Deck XL with the Claude XL profile across 32 keys"><figcaption>Claude XL — 8×4</figcaption></figure>
      <figure class="small"><img src="{img('deck-mini-crop.png')}" width="478" height="390" alt="Stream Deck Mini with the six-key Claude Mini profile"><figcaption>Claude Mini — 3×2</figcaption></figure>
    </div>
  </section>
  ...
  <section id="how">
    ...
    <p class="lead">Nothing runs in the background besides the plugin itself. Each key press travels this path:</p>
    <div class="flow">
      <div class="card"><div class="num">01</div><h3>Stream Deck plugin</h3><p>Node.js, Elgato SDK v2. A key press opens <code>claudedeck://always-allow</code>.</p></div>
      <div class="card"><div class="num">02</div><h3>ClaudeDeck.app</h3><p>A tiny URL-scheme handler in <code>~/Applications</code>, installed by the plugin on first start.</p></div>
  ...
  <section id="install">
    ...
    <div class="grid3">
      <div class="card"><div class="num">1</div><h3>Download and double-click</h3><p><code>com.4xsdev.claude.streamDeckPlugin</code> — Stream Deck asks to install the plugin. It brings the Deck for Claude actions, a ready profile for your deck (Mini, MK.2 or XL), and the helper app.</p></div>
...
<title>Stream Deck for Claude</title>
<meta name="description" content="Stream Deck plugin for the Claude desktop app: answer permission prompts, stop, canned replies, shortcuts and live status on physical keys.">
{FONTS}
<style>{CSS}</style>
</head>
<body>
...
def write_artboards(outdir):
    outdir = pathlib.Path(outdir); outdir.mkdir(parents=True, exist_ok=True)
    for name, theme in (("Main", ""), ("Dark", ' data-theme="dark"')):
        html = f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8">
...
<helmet>
  ...
  .page{{width:1280px;background:var(--bg);color:var(--fg)}}
  </style>
</helmet>
<div class="page"{theme}>
{body(lambda n: n, toggle_button=False)}
</div>
</x-dc>
</body>
</html>
"""
        (outdir / f"{name}.dc.html").write_text(html)

if __name__ == "__main__":
    write_index()
    write_artboards(ROOT / "site" / "canvas")
    print("ok")
